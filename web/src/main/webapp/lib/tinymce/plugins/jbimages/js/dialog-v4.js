...
var jbImagesDialog = {
	...
	inProgress : function() {
		...
		this.timeoutStore = window.setTimeout(function(){
			document.getElementById("upload_additional_info").innerHTML = tinymce.translate('This is taking longer than usual.')
                + '<br />' + tinymce.translate('An error may have occurred.')
                + '<br /><a href="#" onClick="jbImagesDialog.showIframe()">' + tinymce.translate('View script\'s output') + '</a>';
	...
	uploadFinish : function(result) {
		...
		else
		{
			...
			var downloadPath = tinymce.EditorManager.activeEditor.getParam('jbimages_download_path_url');
			var w = this.getWin();
			tinymce = w.tinymce;
			tinymce.EditorManager.activeEditor.insertContent('<img src="' + downloadPath+encodeURIComponent(result.filename) +'">');
            //console.log();
			this.close();
		}
	},
	...
	close : function() {
		...
		function close() {
			...
			tinymce = tinyMCE = t.editor = t.params = t.dom =  null; // Cleanup
    ...
    initForm: function(){
        var w = this.getWin();
        tinymce = w.tinymce;
        var uploadPath = tinymce.EditorManager.activeEditor.getParam('jbimages_upload_path_url');
        document.getElementById("upl").setAttribute("action", uploadPath);
    }
